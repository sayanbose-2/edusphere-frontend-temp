...
const TopNavbar = ({ onToggleSidebar }: Props) => {
  ...
  return (
    <header className="topbar">
      ...
      <span className="text-sm font-semibold text-base ml-1">{pageTitle}</span>
      ...
      <Dropdown align="end">
        ...
        <Dropdown.Menu className="w-80 max-h-96 overflow-y-auto">
          <div className="flex justify-between items-center px-3.5 py-2 border-b border-light">
            <span className="text-xs font-semibold text-base">
              Notifications
            </span>
            {unreadCount > 0 && (
              <button
                onClick={markAllAsRead}
                className="bg-transparent border-0 text-primary-600 text-xs cursor-pointer flex items-center gap-1 font-inherit hover:text-primary-700"
              >
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-base whitespace-normal leading-relaxed">
                      {n.message}
                    </div>
      ...
      <Dropdown align="end">
        <Dropdown.Toggle
          as="button"
          className="bg-transparent border-0 cursor-pointer flex items-center gap-2 px-1 py-0.5 rounded font-inherit hover:bg-base transition-colors"
        >
