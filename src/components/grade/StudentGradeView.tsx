...
import type { IGrade, IExam, ICourse } from "@/types/academicTypes";
...
export const StudentGradeView = ({
  ...
  const examMap: Record<string, IExam> = Object.fromEntries(
    allExams.map((e) => [e.id, e]),
  );
  const courseMap: Record<string, ICourse> = Object.fromEntries(
    courses.map((c) => [c.id, c]),
  );

  const rows: TGradeRow[] = items.map((g) => ({
    grade: g,
    exam: examMap[g.examId],
    course: examMap[g.examId]
      ? courseMap[examMap[g.examId].courseId]
      : undefined,
  }));
  ...
  return (
    <>
      <PageHeader
        title="My Grades"
        subtitle="View your exam results by course"
      />
      {rows.length === 0 ? (
          ...
          {Object.values(grouped).map((courseRows) => {
            ...
            return (
              <div
                ...
                className="card mb-5 p-0 overflow-hidden"
              >
                <div className="py-3 px-5 bg-base border-b border-border">
                  ...
                  {course && (
                    <span className="text-sm text-tertiary ml-2.5">
                ...
                <table className="w-full text-base">
                  <thead>
                    <tr>
                      <th className="py-2 px-5 text-left font-semibold text-secondary text-sm">
                        Exam Type
                      </th>
                      <th className="py-2 px-4 text-left font-semibold text-secondary text-sm">
                        Date
                      </th>
                      <th className="py-2 px-4 text-center font-semibold text-secondary text-sm">
                        Score
                      </th>
                      <th className="py-2 px-4 text-center font-semibold text-secondary text-sm">
                        Grade
                    ...
                    {courseRows.map((r, i) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-4 text-secondary">
                          {r.exam
                            ? new Date(r.exam.date).toLocaleDateString(
                                undefined,
                                {
                                  year: "numeric",
                                  month: "short",
                        ...
                        <td className="py-2.5 px-4 text-center">
                          <span
                            className={`font-black text-lg ${r.grade.grade === "F" ? "text-danger" : r.grade.grade === "A" ? "text-success" : "text-blue"}`}
                          >
                            {r.grade.grade}
                          </span>
                        </td>
