...
const StudentProfileForm = ({ name, onDone, onSignOut }: Props) => {
  ...
  const handleSubmit = async () => {
    if (!dob || !gender || !address.trim()) { toast.error('Please fill in all required fields'); return; }
    setSaving(true);
    try {
      await apiClient.post('/students/me', { dob, gender, address });
    ...
    <div className="min-h-screen flex items-center justify-center bg-base px-4">
      <div className="auth-box max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-secondary m-0">Welcome, {name}! Please fill in a few personal details to get started.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3.5 mb-3.5">
          <div>
            ...
            <DateInput value={dob} onChange={setDob} max={today} required />
          </div>
          <div>
        ...
        <button className="btn btn-primary w-full mb-2.5 font-semibold" onClick={handleSubmit} disabled={saving}>
          {saving && <span className="spinner-border spinner-border-sm me-2" />}
          Save & Go to Dashboard
        </button>
        <button className="btn btn-outline-secondary w-full text-xs" onClick={onSignOut}>Sign out</button>
      </div>
    </div>
  );
};
