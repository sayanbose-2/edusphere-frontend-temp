...
export default function FacultyExams() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [exams, crs] = await Promise.all([examService.getAll(), courseService.getAll()]);
      setItems(exams); setCourses(crs);
  ...
  const openCreate = () => { setSelected(null); setCourseId(''); setType('MIDTERM' as ExamType); setDate(''); setModal('create'); };
  ...
  return (
    <>
      <PageHeader title="My Exams" subtitle="View and manage examinations for your courses"
