...
import { DocumentUploadModal } from "@/components/research/DocumentUploadModal";
...
const DocumentsPage = () => {
    ...
    try {
      if (isStudent) {
        const res = await apiClient.get<
          IPageResponse<IStudentDocument> | IStudentDocument[]
        >("/student-documents/me/docs");
        setData((d) => ({
          ...d,
        ...
        const [docs, studs] = await Promise.all([
          apiClient
            .get<
              IPageResponse<IStudentDocument>
            >("/student-documents/all", { params: { size: 100 } })
  ...
  const handleToggleVerify = async (item: IStudentDocument) => {
    try {
      await apiClient.patch(
        `/student-documents/${item.studentDocumentId}/verify`,
        { verified: !item.verificationStatus },
      );
      toast.success(
        item.verificationStatus ? "Document unverified" : "Document verified",
      );
      load();
    } catch {
  ...
  const handleDownload = async (item: IStudentDocument) => {
    try {
      const res = await apiClient.get(
        `/student-documents/download/${item.studentDocumentId}`,
        { responseType: "blob" },
      );
  ...
  const handleUpload = async (form: IUploadForm) => {
    if (!form.file) {
    ...
    try {
      ...
      await apiClient.post("/student-documents/me/upload", fd, {
  ...
  const columns: Column<IStudentDocument>[] = [
      ...
      ? [
          {
            ...
            label: "Student",
            render: (item: IStudentDocument) => studentName(item.studentId),
          },
        ]
      : []),
    {
      key: "docType",
      label: "Type",
      render: (item) => formatEnum(item.docType),
    },
    ...(!isStudent
      ? [
          {
            key: "downloadUrl" as const,
            label: "File",
            render: (item: IStudentDocument) =>
              item.downloadUrl ? (
                <span title={item.downloadUrl}>
                  {extractFileName(item.downloadUrl)}
                </span>
              ) : (
                "—"
              ),
        ...
        ]),
    {
      ...
      render: (item) => (
        <StatusBadge
          status={item.verificationStatus ? "VERIFIED" : "UNVERIFIED"}
        />
      ),
    },
  ];
      ...
      <PageHeader
        ...
        subtitle={
          isStudent
            ...
            : isFaculty
              ? "Verify or unverify student documents"
              : "View and verify all student documents"
        }
        action={
          isStudent ? (
            <button
