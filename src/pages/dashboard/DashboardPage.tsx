...
const sectionColors: Record<string, string> = {
  ...
  Department: 'bg-purple-100/60 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400',
  System: 'bg-secondary/10 text-secondary',
  Documents: 'bg-orange-100/60 text-orange-600 dark:bg-orange-900/20 dark:text-orange-400',
};
...
const DashboardPage = () => {
  ...
  const menu = getMenuForRole(user.roles).filter(item => item.path !== '/dashboard');
  ...
  return (
    <>
      <div className={`welcome-banner ${bannerClass[primaryRole] ?? 'welcome-banner--admin'} mb-6`}>
      ...
      <div className="flex flex-col gap-6">
        {Object.entries(sections).map(([section, items]) => {
          ...
          return (
            <div key={section}>
              <p className="text-xs font-bold text-tertiary mb-3 uppercase tracking-wider">{section}</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {items.map(item => {
                  const Icon = (BsIcons as Record<string, React.ElementType>)[item.icon];
                  return (
                    <div
                      key={item.path}
                    ...
                    >
                      ...
                      <p className="m-0 font-semibold text-sm text-base">{item.label}</p>
                    </div>
                  );
                })}
              </div>
