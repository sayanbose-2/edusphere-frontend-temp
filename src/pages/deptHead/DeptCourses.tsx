...
export default function DeptCourses() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (found) {
        setItems(allCourses.filter((c) => c.departmentId === found.id));
      } else {
        setItems(allCourses);
      }
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await courseService.delete(id);      toast.success('Course deleted');
      fetchData();
    } catch {
  ...
  return (
    <div>
      <PageHeader
        title="Courses"
        subtitle={myDept ? `Courses in ${myDept.departmentName}` : 'Manage courses'}
        action={
