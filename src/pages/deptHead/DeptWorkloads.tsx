...
export default function DeptWorkloads() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (myDept) {
        ...
        const deptFacultyIds = new Set(deptFaculties.map((f) => f.id));
        setItems(allWorkloads.filter((w) => deptFacultyIds.has(w.facultyId)));
      } else {
        setFaculties(allFaculties);
        setCourses(allCourses);
        setItems(allWorkloads);
      }
    } catch {
  ...
  return (
    <div>
      <PageHeader
        title="Workloads"
        subtitle="Manage faculty workloads for your department"
        action={
