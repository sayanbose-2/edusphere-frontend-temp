...
import { ZodLoginSchema, type ILoginForm } from "@/zod/auth/ZodAuthSchema";

type TForm = ILoginForm;
...
const LoginPage = () => {
  ...
  const onSubmit = async (data: TForm) => {
    try {
      ...
      const needsProfileCheck = roles.some(
        (r) => r === Role.STUDENT || r === Role.FACULTY,
      );
      navigate(needsProfileCheck ? "/profile/setup" : "/dashboard", {
        replace: true,
      });
    } catch (err: unknown) {
      const msg =
    ...
    <div className="auth-page">
      <div className="auth-left">
        <div className="relative z-10 text-center max-w-xs">
          ...
          <h2 className="text-blue-50 font-black text-2xl mb-2 tracking-tight">
            EduSphere
          </h2>
          <p className="text-blue-50/50 text-sm mb-11 leading-relaxed">
          ...
          <div className="flex flex-col gap-4 text-left">
            {[
              {
                icon: <BsPeopleFill size={14} />,
                title: "Multi-Role Access",
                desc: "Students, Faculty, Admins & more",
              },
              {
                icon: <BsBook size={14} />,
                title: "Academic Records",
                desc: "Grades, courses, and curriculum",
              },
              {
                icon: <BsAwardFill size={14} />,
                title: "Research & Thesis",
                desc: "End-to-end project tracking",
              },
            ].map((item) => (
              <div key={item.title} className="flex gap-3 items-start">
                ...
                <div>
                  <div className="text-blue-50 font-semibold text-sm">
                    {item.title}
                  </div>
                  <div className="text-blue-50/40 text-xs mt-0.5">
                    {item.desc}
                  </div>
                </div>
              </div>
