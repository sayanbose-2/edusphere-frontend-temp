...
export default function GradeList() {
  ...
  const fetchByStudent = async (sid: string) => {
    if (!sid) { fetchData(); return; }
    try {
      setLoading(true);
      setItems(await gradeService.getByStudent(sid));
    } catch {
      toast.error('Failed to filter by student');
    } finally {
      setLoading(false);
    }
  };

  const fetchByExam = async (eid: string) => {
    if (!eid) { fetchData(); return; }
    try {
      setLoading(true);
      setItems(await gradeService.getByExam(eid));
    } catch {
      toast.error('Failed to filter by exam');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getExamLabel = (id: string) => {
    ...
    return exam ? `${getCourseName(exam.courseId)} — ${formatEnum(exam.type)}` : '—';
  };
    ...
    <div>
      ...
      <div className="d-flex gap-3 mb-3">
        <Form.Group className="d-flex align-items-center gap-2">
          <Form.Label className="mb-0 small text-nowrap">Filter by Student:</Form.Label>
          <Form.Select
            size="sm"
        ...
        <Form.Group className="d-flex align-items-center gap-2">
          <Form.Label className="mb-0 small text-nowrap">Filter by Exam:</Form.Label>
          <Form.Select
            size="sm"
            value={filterExamId}
            onChange={(e) => {
              ...
              setFilterStudentId('');
              fetchByExam(e.target.value);
            }}
          >
            <option value="">All Exams</option>
            {exams.map((ex) => (
              <option key={ex.id} value={ex.id}>{getCourseName(ex.courseId)} - {ex.type}</option>
            ))}
          </Form.Select>
        </Form.Group>
      </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>{editItem ? 'Edit Grade' : 'Create Grade'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control value={grade} onChange={(e) => setGrade(e.target.value)} placeholder="e.g. A, B, C" />
            </Form.Group>
            <Form.Group className="mb-3">
